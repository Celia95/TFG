...
\begin{proof}
    ...
    Así,
    \begin{equation*}
        h(z) = z + h(z^2) = z + z^2 + h(z^4) = \cdots = z + z^2 + \cdots + z^{2^{k-1}} + h(z^{2^k}). % El último término debería ir con h(algo)
    \end{equation*}
...
\begin{example}
    ...
    \end{equation*}
    es holomorga, $\abs{f(z)} \leq 1$ para todo $z \in \disk$, y $f(t) \rightarrow 0$ cuando $t \rightarrow 1^-$.
\end{example}
...
\begin{proof}
    ...
    \begin{equation*}
        \abs{e^z} = \abs{e^{x+iy}} = \abs{e^{x}(\cos y + i\sen y)} = e^{x} < 1. % creo que aquí pondría e^x, sin barras y sin que aparezca la variable   y
    \end{equation*}

    La aplicación $g$ es una transformación de Möbius, y las transformadas de Möbius tienen la propiedad de que llevan circunferencias y rectas en circunferencias y rectas. Como la función lleva $-1$ a $0$, $i$ a $-i$ y $-i$ a $i$, la imagen del círculo $\abs{z}=1$, ha de ser una recta. \\ % Extender esto
