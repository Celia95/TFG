...
\begin{proof}
    ...
    \begin{equation*}
     ...
     % Falta comentar que la serie armónica diverge
...
\begin{proof}
    ...
    Así,
    \begin{equation*}
        h(z) = z + h(z^2) = z + z^2 + h(z^4) = \cdots = z + z^2 + \cdots + z^{2^{k-1}} + z^{2^k}. % El último término debería ir con h(algo)
    \end{equation*}
...
\begin{example}
    ...
    \end{equation*}
    es analítica, $\abs{f(z)} \leq 1$ para todo $z \in \disk$, y $f(t) \rightarrow 0$ cuando $t \rightarrow 1^-$.
\end{example}
...
\begin{proof}
    ...
    \begin{equation*}
        \abs{e^z} = \abs{e^{x+iy}} = \abs{e^{x}(\cos y + i\sen y)} = \abs{e^{x}\cdot e^{iy}} < 1.
    \end{equation*}

    La aplicación $g$ es una transformada de Möbius, y las transformadas de Möbius tienen la propiedad de que llevan círculos a líneas o círculos. Como la función lleva $-1$ a $0$, $i$ a $-i$ y $-i$ a $i$, la imagen del círculo $\abs{z}=1$, ha de ser una línea. \\ % Extender esto

    Si tomamos la sucesión $\{t_n\}$ de término general $t$,
    \begin{equation*}
        \frac{t + 1}{t - 1} \xrightarrow[t \rightarrow 1^-]{}  - \infty \Rightarrow \exp \left(  \frac{t + 1}{t - 1} \right) \xrightarrow[t \rightarrow 1^-]{} 0.
    \end{equation*}

     Si tomamos la sucesión $\{z_n\}$ definida por $z_n = g(w_n)$, siendo $\{w_n\}$ la sucesión de término general $-1 + 2n \pi i$. Entonces,
     \begin{equation*}
