...
\todo[inline, color=yellow]{Hablar más de las evaluaciones. ¿Qué más hay que decir?}
...
\todo[inline, color=yellow]{Falta ver que $\pi^{-1}$ aplica homeomórficamente $\disk$ sobre un abierto de $\fiber$. Lo encontré en el Hoffman, yo tampoco lo entiendo. ¿Lo quito?}
...
\todo[inline, color=yellow]{La definición anterior hace que podamos descomponer el espectro como unión disjunta de sus fibras. ¿Cómo hacemos esto?}

\todo[inline, color=yellow]{Observemos que la imagen de toda función constante por cualquier elemento del espectro es ella misma. Además, la identidad es una función de $\bholomorphic{\disk}$ de norma 1.}
...
\section{Estructura topológica de $\fiber$ / Otro título}
...
El Teorema anterior nos muestra que a pesar del Teorema de Fatou, el comportamiento en los puntos de la frontera puede ser impredecible. Por ejemplo, si consideramos un producto de Blaschke cuyos ceros sean densos en $\partial \disk$, entonces el Teorema se cumple para todo $z \in \partial \disk$. Sin embargo, la función tendrá límites radiales en casi todo punto. \\


\todo[inline]{Aquí vendría bien citar lo que pasa con la función singular, para la que se puede calcular explícitamente $Cl(f,1)$. Y también hablar de los productos de Blaschke: decir que la sucesión $(\alpha_n)$ tiene puntos límite en el borde del disco, en los que entonces $B$ no va a tener límite y eso da lugar a puntos $w$ donde $Cl(B,w)$ es todo el disco cerrado.}
