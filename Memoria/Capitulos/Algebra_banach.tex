...
\todo[inline, color=yellow]{Mirar esto con cuidado. Creo que $\Delta = \pi^{-1} (\disk) = \{\delta_z : z \in \disk\}$, no?}
...
Ya conocemos funciones internas tales como los automorfismos del disco unidad o los productos de Blaschke, de los que ya hemos hablado en el Capítulo \ref{tal}. %Citar en concreto los resultados 3.3.1 y el corolario 3.3.1.1.
Además de este tipo de funciones, la función $S(z) = \exp{\left(\frac{z + 1}{z - 1}\right)}, z \in \disk$, que introdujimos en el Ejemplo \ref{ex:exp} también es una función interna. Recordemos que si $\abs{w} = 1$ y $w \neq 1$, entonces $S$ puede extenderse a $w$ $\abs{S(w)} = 1$. \\
...
Las transformaciones de Möbius tienen la propiedad de que llevan circunferencias y rectas en circunferencias y rectas. Por lo que $T$ lleva el eje real en sí mismo, mientras que cada circunferencia con centro en el eje real que pasa por $1$ se aplica sobre una recta paralela al eje imaginario, como podemos ver en la figura \ref{fig:exp}. En concreto, si la circunferencia $C_t$ tiene diámetro $[t, 1]$, su imagen es la recta
\begin{equation}
    \{z \in \complex : \Re(z) = \frac{t + 1}{t - 1}\}
\end{equation}
Usamos este hecho para observar que cada $w \in \disk$ es límite de una sucesión $\{S(z_n)\}$, con $\lim_{n \to \infty} z_n = 1$. Veamos esto último, distinguiendo tres casos:
...
\begin{figure}[h]{}
    ...
    \begin{tikzpicture}[scale=2]
        ...
        \fill[color=cccccc,fill=cccccc,fill opacity=0.3499999940395355] (0, 1.5) -- (-3.5, 1.5) -- (-3.5, -1.5) -- (0, -1.5) -- cycle;
